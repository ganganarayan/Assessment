...
for (const type of ACTIVE_EVENT_TYPES) {
  ...
  const expectedTop = type === EventType.LEAD_CREATED ? [...TOP, "contact.assess_profession"] : TOP;
  expect(`${name} · top-level keys`, keysEq(env, expectedTop), JSON.stringify(Object.keys(env)));
  if (type === EventType.LEAD_CREATED) {
    expect(`${name} · has contact.assess_profession`, "contact.assess_profession" in env);
  } else {
    expect(`${name} · no contact.assess_profession`, !("contact.assess_profession" in env));
  }
...
// Profession → contact.assess_profession, lead.created ONLY.
{
  ...
  expect("lead.created carries profession value", led["contact.assess_profession"] === "Business Owner");
  const ledNull = buildEnvelope(EventType.LEAD_CREATED, { submissionId: SID, lead: { firstName: "G" } }, BASE);
  expect("lead.created profession null when absent", ledNull["contact.assess_profession"] === null);
  const comp = buildEnvelope(EventType.ASSESSMENT_COMPLETED, { submissionId: SID, lead: leadProf }, BASE);
  expect("completed omits profession field", !("contact.assess_profession" in comp));
  const started = buildEnvelope(EventType.ASSESSMENT_STARTED, { submissionId: SID, lead: leadProf }, BASE);
  expect("started omits profession field", !("contact.assess_profession" in started));
}
...
// Per-event payload shaping (lead.created drops nulls; assessment.started minimal).
{
  ...
  expect("lead: drops null profession", !("contact.assess_profession" in led));
  ...
  expect("lead: keeps profession when present", ledProf["contact.assess_profession"] === "Business Owner");
