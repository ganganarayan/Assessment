...
/**
 * Enqueue COMPLETED contacts of an assessment for a CUSTOM broadcast.
 * scope="paid" restricts to contacts with a captured payment (source of truth for
 * "paid" — independent of the completedPaidAt stamp, so it still catches sales whose
 * stamp never fired). scope="all" = every completed contact (default, back-compat).
 */
export async function enqueueCustom(assessmentId: string, scope: CustomScope = "all"): Promise<number> {
  ...
  if (scope === "paid") {
    ...
    paidIds = [...new Set(payments.map((p) => p.submissionId).filter((x): x is string => Boolean(x)))];
