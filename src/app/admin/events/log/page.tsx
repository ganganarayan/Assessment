...
export default async function EventLogPage({
  ...
  const link = (p: number) =>
    `/admin/events/log?${new URLSearchParams({
      ...(type ? { type } : {}),
      page: String(p),
    }).toString()}`;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/admin/events/log" className={buttonVariants({ variant: type ? "outline" : "default", size: "sm" })}>
          All ({totalAll})
        </Link>
        {ALL_EVENT_TYPES.map((t) => (
          <Link
            key={t}
            href={`/admin/events/log?type=${t}`}
