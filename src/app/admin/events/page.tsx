  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-[var(--muted-foreground)]">{totalAll} total</span>
      </div>

      <div className="flex flex-wrap gap-2">
        <Link href="/admin/events" className={buttonVariants({ variant: type ? "outline" : "default", size: "sm" })}>
          All ({totalAll})
        </Link>
        {ALL_EVENT_TYPES.map((t) => (
          <Link
            ...
            className={buttonVariants({ variant: type === t ? "default" : "outline", size: "sm" })}
          >
          ...
          <table className="w-full text-sm">
            <thead className="bg-[var(--muted)] text-left text-xs text-[var(--muted-foreground)]">
              <tr>
                ...
                <th className="px-3 py-1.5">Submission</th>
            ...
            <tbody className="divide-y">
              {rows.map((e) => (
                <tr key={e.id}>
                  <td className="whitespace-nowrap px-3 py-1.5 text-xs">{fmt(e.createdAt)}</td>
                  <td className="px-3 py-1.5 font-mono text-xs">{e.name}</td>
                  <td className="px-3 py-1.5 font-mono text-xs">{e.submissionId ?? "—"}</td>
                  <td className="px-3 py-1.5">{e.leadEmail ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          {page > 1 ? (
            <Link href={link(page - 1)} className={buttonVariants({ variant: "outline", size: "sm" })}>Previous</Link>
          ) : null}
          {page < pages ? (
            <Link href={link(page + 1)} className={buttonVariants({ variant: "outline", size: "sm" })}>Next</Link>
          ) : null}
        </div>
      </div>
    </div>
